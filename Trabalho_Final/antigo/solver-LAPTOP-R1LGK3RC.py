...
# Parâmetros
L = {t: data["tasks"][t]["duration"]//Duration_per_slot for t in T}
...
# Proibir inícios inválidos: se s + L[t] > S então y[...] == 0
for p in P:
    for t in T:
        dur = L[t]
        last_start = S - dur  # último slot válido para iniciar t
        for i in I[t]:
            for s in range(S):
                if s > last_start:
                    model += y[p][t][i][s] == 0
...
for s in range(S):
    ...
    for p in P:
        for t in T:
            ...
            s_start_max = min(s, S - dur)  # início não pode ultrapassar last_start
            for i in I[t]:
...
for t in T:
    for i in I[t]:
        model += pulp.lpSum(y[p][t][i][s] for p in P for s in range(S)) == 1
